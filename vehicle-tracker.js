        ...
        try {
            ...
            if (unit) {
                ...
                if (trackerMarkers[unit.id]) {
                    ...
                    var popupContent = '<div class="tracker-map-popup">' +
                        '<div class="popup-name">🚛 ' + unit.name + '</div>' +
                        '<div class="popup-speed" style="color:' + speedColor + '">' + Math.round(unit.speed) + ' km/h</div>' +
                        '<div class="popup-detail">' +
                        '📍 <strong>' + unit.address + '</strong><br>' +
                        '📡 ' + unit.satellites + ' satellites<br>' +
                        '⏱️ ' + timeAgo(unit.lastTime) +
                        '</div>' +
                        '<div class="popup-coords">' + unit.lat.toFixed(5) + ', ' + unit.lng.toFixed(5) + '</div>' +
                        '</div>';
    ...
    function renderTrackerCards(units) {
        ...
        filtered.forEach(function (unit, i) {
            ...
            cardsHtml += '<div class="tracker-vehicle-card status-' + status + '" style="animation-delay: ' + (i * 0.05) + 's" data-unit-id="' + unit.id + '">' +
                ...
                '</div>' +
                '<div class="tracker-card-speed" style="display:flex; flex-direction:column; align-items:flex-start; width:100%;">'+
                '<div style="display:flex; align-items:baseline; gap:8px;">' +
                '<span class="tracker-speed-value ' + speedClass + '">' + Math.round(unit.speed) + '</span>' +
                '<span class="tracker-speed-unit">km/h</span>' +
                '</div>' +
